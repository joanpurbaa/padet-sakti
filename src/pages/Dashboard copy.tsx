								...
								<PieChart>
									...
									<Tooltip
										...
										formatter={(value, name) => {
											const label = String(name).split("(")[0].trim();
											return [`${value} kejadian`, label];
										}}
									/>
									<Legend
										iconType="circle"
										...
										formatter={(value) => {
											...
											const match = strValue.match(/IB (\w+)/);
											const status = match ? match[1] : strValue;
											const count = strValue.match(/\d+/)?.[0];
											const percent =
