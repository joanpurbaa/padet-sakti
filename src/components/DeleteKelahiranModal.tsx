...
import { deleteKelahiran } from "../service/kelahiranService";
import type { KejadianDetailKelahiran } from "../types/Kejadian";

interface DeleteKelahiranModalProps {
	open: boolean;
	onClose: () => void;
	onSuccess: () => void;
...
export default function DeleteKelahiranModal({
	open,
	onClose,
	onSuccess,
	kelahiran,
}: DeleteKelahiranModalProps) {
	...
	const handleDelete = async () => {
		...
		try {
			await deleteKelahiran(kelahiran.id_kelahiran);
			onSuccess();
			onClose();
		} catch (err: unknown) {
		...
		<div
			...
			<div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
				<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
					<h2 className="text-lg font-bold text-gray-800">Hapus Kelahiran</h2>
					<button
						onClick={onClose}
						disabled={loading}
					...
					<div className="flex items-start gap-3">
						...
						<div>
							<p className="text-sm text-gray-700">
								Apakah kamu yakin ingin menghapus data kelahiran ini?
							</p>
							<p className="text-sm font-semibold text-gray-900 mt-1">
								{kelahiran.id_kelahiran}
							</p>
							<p className="text-xs text-gray-400 mt-0.5">
								Nama: {kelahiran.nama} · {kelahiran.jenis_kelamin} · Staff:{" "}
								{kelahiran.id_staff}
							</p>
						</div>
					</div>
