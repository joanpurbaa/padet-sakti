...
export default function IBFormModal({
	open,
	onClose,
	onSuccess,
	ib,
	...
	const [form, setForm] = useState<IBForm>(EMPTY_FORM);
	...
	const [kejadianLoading, setKejadianLoading] = useState(false);
	...
	useEffect(() => {
		...
		getKejadian({ page: 1 }, controller.signal)
			...
			.catch((err) => {
				if (err.name !== "AbortError") {
					console.error("Failed to load kejadian:", err);
				}
			})
			.finally(() => setKejadianLoading(false));
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (isEdit && ib) {
				await editIB(ib.id_ib, payload);
			} else {
				await addIB(form.kejadian, payload);
			}
			onSuccess();
			onClose();
		} catch (err: unknown) {
			if (err instanceof Error) {
				...
				setGeneralError(
					isEdit ? "Gagal mengubah data IB" : "Gagal menambahkan data IB",
				);
			}
		} finally {
			setLoading(false);
		...
		<div
			...
			<div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
				<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
					<div>
						<h2 className="text-lg font-bold text-gray-800">
							{isEdit ? "Edit Inseminasi Buatan" : "Tambah Inseminasi Buatan"}
						</h2>
						{isEdit && ib ? (
							<p className="text-xs text-gray-400 mt-0.5">{ib.id_ib}</p>
						) : (
							<p className="text-xs text-gray-400 mt-0.5">Home / Add_ib</p>
						)}
					</div>
					<button
						onClick={onClose}
				...
				<form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
					...
					<FormField label="Id Kejadian" required error={errors.kejadian}>
						{isEdit ? (
							<input
								type="text"
								value={form.kejadian}
