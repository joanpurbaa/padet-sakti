...
import { deletePejantan } from "../service/pejantanService";
import type { Pejantan } from "../types/Pejantan";

interface DeletePejantanModalProps {
	open: boolean;
	onClose: () => void;
	onSuccess: () => void;
...
export default function DeletePejantanModal({
	open,
	onClose,
	onSuccess,
	pejantan,
}: DeletePejantanModalProps) {
	const [loading, setLoading] = useState(false);
	const [error, setError] = useState<string | null>(null);

	if (!open || !pejantan) return null;
	...
	const handleDelete = async () => {
		...
		try {
			await deletePejantan(pejantan.id_pejantan);
			onSuccess();
			onClose();
		} catch (err: unknown) {
			if (err instanceof Error) {
				...
				setError("Gagal menghapus data pejantan");
			}
		} finally {
			setLoading(false);
		}
		...
		<div
			...
			<div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
				<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
					<h2 className="text-lg font-bold text-gray-800">Hapus Pejantan</h2>
					<button
						onClick={onClose}
						disabled={loading}
					...
					<div className="flex items-start gap-3">
						...
						<div>
							<p className="text-sm text-gray-700">
								Apakah kamu yakin ingin menghapus data pejantan ini?
							</p>
							<p className="text-sm font-semibold text-gray-900 mt-1">
								{pejantan.id_pejantan}
							</p>
							<p className="text-xs text-gray-400 mt-0.5">
								Jenis: {pejantan.jenis_straw} · Asal: {pejantan.asal_straw} · Pembuatan:{" "}
								{pejantan.id_pembuatan}
							</p>
						</div>
					</div>
