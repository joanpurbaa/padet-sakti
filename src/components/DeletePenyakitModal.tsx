...
import { deletePenyakit } from "../service/penyakitService";
import type { PenyakitModalProps } from "../types/Penyakit";

export default function DeletePenyakitModal({
	open,
	onClose,
	onSuccess,
	penyakit,
	...
	const handleDelete = async () => {
		...
		try {
			await deletePenyakit(penyakit.id_penyakit);
			onSuccess();
			onClose();
		} catch (err: unknown) {
			if (err instanceof Error) {
				...
				setError("Gagal menghapus data penyakit");
			}
		} finally {
			setLoading(false);
		}
		...
		<div
			...
			<div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
				<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
					<h2 className="text-lg font-bold text-gray-800">Hapus Data Penyakit</h2>
					<button
						onClick={onClose}
						disabled={loading}
					...
					<div className="flex gap-3">
						...
						<div>
							...
							<p className="text-xs text-gray-400 mt-0.5">
								Nama Ternak: {penyakit.signalment_nama}
							</p>
