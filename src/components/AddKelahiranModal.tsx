...
const EMPTY_FORM: KelahiranForm = {
	staff: "",
	ticket: "",
	nama: "",
	kelamin: "Jantan",
...
export default function AddKelahiranModal({
	...
	onSuccess,
	idKejadian,
}: AddKelahiranModalProps) {
	const [form, setForm] = useState<KelahiranForm>(EMPTY_FORM);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (err instanceof Error) {
				...
				setGeneralError("Gagal menambahkan data kelahiran");
			}
		} finally {
			setLoading(false);
		}
		...
		<div
			...
			<div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
				<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
					<div>
						<h2 className="text-lg font-bold text-gray-800">Tambah Kelahiran</h2>
						<p className="text-xs text-gray-400 mt-0.5">
							Home / Add_kelahiran / {idKejadian}
						</p>
					</div>
					<button
						onClick={onClose}
