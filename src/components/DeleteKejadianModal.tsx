...
import type { DeleteKejadianModalProps } from "../types/Kejadian";
import { deleteKejadian } from "../service/kejadianService";

export default function DeleteKejadianModal({
	open,
	onClose,
	onSuccess,
	kejadian,
}: DeleteKejadianModalProps) {
	const [loading, setLoading] = useState(false);
	const [error, setError] = useState<string | null>(null);

	if (!open || !kejadian) return null;
	...
	const handleDelete = async () => {
		...
		try {
			await deleteKejadian(kejadian.id_kejadian);
			onSuccess();
			onClose();
		} catch (err: unknown) {
			if (err instanceof Error) {
				...
				setError("Gagal menghapus kejadian");
			}
		} finally {
			setLoading(false);
		}
		...
		<div
			...
			<div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
				<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
					<h2 className="text-lg font-bold text-gray-800">Hapus Kejadian</h2>
					<button
						onClick={onClose}
						disabled={loading}
					...
					<div className="flex gap-3">
						...
						<div>
							...
							<p className="text-xs text-gray-400 mt-0.5">
								Peternak: {kejadian.id_peternak} · Betina: {kejadian.id_betina}
							</p>
