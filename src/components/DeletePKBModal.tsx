...
import { deletePKB } from "../service/pkbService";
import type { KejadianDetailPKB } from "../types/Kejadian";

interface DeletePKBModalProps {
	open: boolean;
	onClose: () => void;
	onSuccess: () => void;
...
export default function DeletePKBModal({
	open,
	onClose,
	onSuccess,
	pkb,
	...
	const handleDelete = async () => {
		...
		try {
			...
			if (err instanceof Error) {
				...
				setError("Gagal menghapus data PKB");
			}
		} finally {
			setLoading(false);
		}
		...
		<div
			...
			<div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
				<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
					<h2 className="text-lg font-bold text-gray-800">Hapus PKB</h2>
					<button
						onClick={onClose}
						disabled={loading}
					...
					<div className="flex items-start gap-3">
						...
						<div>
							<p className="text-sm text-gray-700">
								Apakah kamu yakin ingin menghapus data PKB ini?
							</p>
							<p className="text-sm font-semibold text-gray-900 mt-1">{pkb.id_pkb}</p>
							<p className="text-xs text-gray-400 mt-0.5">
								IB: {pkb.id_ib} · Staff: {pkb.id_staff} · Hasil: {pkb.hasil}
							</p>
						</div>
					</div>
